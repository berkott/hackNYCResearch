...
def make_sphere_vis(model_strings):
    ...
    for sample_idx in range(SUBSAMPLE_N):
        ...
        layout = go.Layout(scene=dict(xaxis_title='X', yaxis_title='Y', zaxis_title='Z'),
                           ...
                           title=" ".join(graph_name))

        # print('at: ', all_traces)
        print(f"Figure {sample_idx}")
...
def create_atlas():
    ...
    documents = [{
            ...
            'url': f'https://berkott.github.io/hackNYCResearch/generate_dataset/figs/3d_scatter_plot_sampled_labels_{i}.html', # Assumes you have already pushed everythin to GH
            "id": str(i)
...
def make_sphere_data(model_strings):
    if os.path.exists('./data/figure_sphere_data.pkl'):
    ...
    for model_string in model_strings:
        ...
        with open('./data/' + save_string + '_fig_embeddings.pkl', 'rb') as f:
            all_embs = pickle.load(f)
            for prompt in all_embs:
                for sequence in prompt:
                    sequence = np.squeeze(sequence)
                    sequence = np.stack(sequence).astype('float64')
                    sequence = sequence / (np.linalg.norm(sequence, axis=1, keepdims=True) + 0.000000000001)
    ...
    print(pre_umap.shape)
    print("preumap")

    # precomputed_knn = nearest_neighbors(
    #     pre_umap, n_neighbors = 50000, metric="euclidean",
    #     metric_kwds=None, angular=False, random_state=1)

    # sphere_mapper = UMAP(precomputed_knn=precomputed_knn, output_metric='haversine', verbose=True).fit(pre_umap)
    ...
    with open('./data/figure_sphere_data.pkl', 'wb') as f:
        pickle.dump(sphere_data_by_model_string, f)


def get_data(model_string):
    save_string = model_string.replace('/', '_')
    if os.path.exists('./data/' + save_string + '_fig_embeddings.pkl'):
    ...
    for e in tqdm(chosen_ds):
        ...
        try:
            ...
            cur_embeddings = np.zeros_like(all_embeddings[-1])
            print("REUSING EMBEDDING DIMS FROM LAST TIME")
    ...
    with open('./data/' + save_string + '_fig_embeddings.pkl', 'wb') as f:
        pickle.dump(all_embeddings, f)
    with open('./data/' + save_string + '_fig_generations.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    make_csv(["EleutherAI/gpt-j-6B", "reciprocate/ppo_hh_gpt-j"])
    create_atlas()